...
public class ManagingDirectorWorkLinksActivity extends AppCompatActivity {

    private static final String TAG = "ManagingDirectorWorkLinks";
    ...
    private LinearLayout workLinksGrid;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_managing_director_work_links);
    ...
    private void initializeViews() {
        ...
        workLinksGrid = findViewById(R.id.workLinksGrid);
    ...
    private void getUserData() {
        ...
        if (intent != null) {
            ...
            if (userName != null && !userName.isEmpty()) {
                welcomeText.setText("Welcome to Work Links, " + userName + "!");
            }
        }
    }

    private void fetchWorkIcons() {
        showLoading(true);
        hideError();
        
        // First, get the work_icons from tbl_user for the current user
        String userUrl = "https://emp.kfinone.com/mobile/api/get_user_work_icons.php?user_id=" + userId;
                ...
                public void onResponse(JSONObject response) {
                    try {
                        if (response.optBoolean("success", false)) {
                            ...
                            String workIconsStr = userData.optString("work_icons", "");
                            
                            if (!workIconsStr.isEmpty()) {
                                try {
                                    // Parse the work_icons JSON array
                                    JSONArray iconIdsArray = new JSONArray(workIconsStr);
                                    ...
                                    String[] iconIds = workIconsStr.split(",");
                                    fetchIconDetails(iconIds);
                                }
                            } else {
                                showError("No work icons found for this user");
                            }
                        } else {
    ...
    private void fetchIconDetails(String[] iconIds) {
        ...
        String iconsUrl = "https://emp.kfinone.com/mobile/api/get_work_icons.php?ids=" + idsBuilder.toString();
        
        // Log the URL for debugging
        Log.d(TAG, "Fetching work icons from URL: " + iconsUrl);
        ...
        JsonObjectRequest iconsRequest = new JsonObjectRequest(Request.Method.GET, iconsUrl, null,
                ...
                public void onResponse(JSONObject response) {
                    try {
                        if (response.optBoolean("success", false)) {
                            ...
                            Log.d(TAG, "Received " + iconsArray.length() + " work icons from API");
                            
                            for (int i = 0; i < iconsArray.length(); i++) {
                                JSONObject iconData = iconsArray.getJSONObject(i);
                                
                                Log.d(TAG, "Work Icon " + i + ": " + iconData.toString());
                                ...
                                Log.d(TAG, "Created WorkIcon: " + icon.getIconName() + " - " + icon.getIconDescription());
                                workIcons.add(icon);
                            }
                            
                            Log.d(TAG, "Total WorkIcon objects created: " + workIcons.size());
                            displayWorkIcons();
                        } else {
                            String errorMsg = response.optString("error", "Unknown error occurred");
                            showError("Error fetching work icons: " + errorMsg);
                        }
                    } catch (JSONException e) {
            ...
            new Response.ErrorListener() {
                @Override
                public void onErrorResponse(VolleyError error) {
                    Log.e(TAG, "Error fetching work icons: " + error.getMessage());
                    showError("Error fetching work icons");
                }
            }
        );
    ...
    private void displayWorkIcons() {
        if (workIcons == null || workIcons.isEmpty()) {
            showError("No work icons available");
            return;
        }
        
        Log.d(TAG, "Displaying " + workIcons.size() + " work icons");
        
        workLinksGrid.removeAllViews();
        
        // Create a 2-column grid using LinearLayouts
        LinearLayout currentRow = null;
        int activeCount = 0;
        
        for (int i = 0; i < workIcons.size(); i++) {
            WorkIcon icon = workIcons.get(i);
            Log.d(TAG, "Work Icon: " + icon.getIconName() + " - " + icon.getIconDescription());
            ...
            if (i % 2 == 0) {
                ...
                workLinksGrid.addView(currentRow);
            }
        ...
        linksCountText.setText("Total Work Links: " + activeCount);
        showLoading(false);
        
        // Make sure the grid is visible
        workLinksGrid.setVisibility(View.VISIBLE);
        errorMessage.setVisibility(View.GONE);
        
        Log.d(TAG, "Display completed. Added " + activeCount + " work icons to grid");
    }

    private CardView createIconCard(WorkIcon icon) {
    ...
    private void openIconUrl(WorkIcon icon) {
    ...
    private void showLoading(boolean show) {
        if (show) {
            ...
            workLinksGrid.setVisibility(View.GONE);
            errorMessage.setVisibility(View.GONE);
        } else {
    ...
    private void addTestCards() {
        Log.d(TAG, "Adding test work cards to grid");
        workLinksGrid.removeAllViews();
        ...
        // Create test WorkIcon objects
        WorkIcon testIcon1 = new WorkIcon("1", "Test Work Icon 1", "https://example.com", "", "Test Work Description 1");
        WorkIcon testIcon2 = new WorkIcon("2", "Test Work Icon 2", "https://example.com", "", "Test Work Description 2");
        ...
        workLinksGrid.addView(testRow);
        
        workLinksGrid.setVisibility(View.VISIBLE);
        errorMessage.setVisibility(View.GONE);
        
        Log.d(TAG, "Test work cards added. Grid should now be visible.");
    }
