...
public class ChiefBusinessOfficerDataLinksActivity extends AppCompatActivity {

    private static final String TAG = "ChiefBusinessOfficerDataLinksActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_chief_business_officer_data_links);
    ...
    private void loadDataIcons() {
        executor.execute(() -> {
            try {
                // First, get the data_icons IDs from tbl_user for the current user
                String userUrl = BASE_URL + "get_chief_business_officer_user_data_icons.php";
                ...
                if (userResponse != null) {
                    ...
                    if (userJson.getBoolean("success")) {
                        ...
                        // Parse the data_icons - it might be a JSON array string or comma-separated
                        List<String> iconIdList = new ArrayList<>();
                        
                        if (dataIconsIds.startsWith("[") && dataIconsIds.endsWith("]")) {
                            // It's a JSON array string, parse it
                            try {
                                JSONArray iconIdsArray = new JSONArray(dataIconsIds);
                                ...
                                Log.e(TAG, "Error parsing JSON array from data_icons", e);
                            }
                        } else {
    ...
    private void fetchDataIconDetails(List<String> iconIds) {
        executor.execute(() -> {
            try {
                ...
                String iconUrl = BASE_URL + "get_chief_business_officer_data_icon_details.php";
