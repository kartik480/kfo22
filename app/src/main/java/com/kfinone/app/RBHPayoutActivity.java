...
public class RBHPayoutActivity extends AppCompatActivity {
    private static final String TAG = "RBHPayoutActivity";
    ...
    private void loadPayoutList() {
        executor.execute(() -> {
            try {
                ...
                if (responseCode == HttpURLConnection.HTTP_OK) {
                    ...
                    if (jsonResponse.getBoolean("success")) {
                        ...
                        for (int i = 0; i < dataArray.length(); i++) {
                            ...
                            String payoutType = item.optString("payout_type_name", "RBH Payout");
                ...
                runOnUiThread(() -> {
                    Toast.makeText(RBHPayoutActivity.this, "Error loading payouts: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                });
            }
        });
    }
