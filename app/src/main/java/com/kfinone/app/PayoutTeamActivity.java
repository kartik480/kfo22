...
public class PayoutTeamActivity extends AppCompatActivity {
    private static final String TAG = "PayoutTeamActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_payout_team);
    ...
    private void loadPayoutList() {
        executor.execute(() -> {
            try {
                String urlString = BASE_URL + "get_payout_team_payouts.php";
                URL url = new URL(urlString);
                HttpURLConnection connection = (HttpURLConnection) url.openConnection();
                connection.setRequestMethod("GET");
                connection.setConnectTimeout(10000);
                ...
                if (responseCode == HttpURLConnection.HTTP_OK) {
                    ...
                    Log.e(TAG, "Payout Team Payouts API response: " + response.toString());
                    ...
                    if (jsonResponse.getBoolean("success")) {
                        ...
                        for (int i = 0; i < dataArray.length(); i++) {
                            ...
                            String payoutType = item.optString("payout_type_name", "Payout Team");
                        ...
                        Log.e(TAG, "Payout Team Payouts API failed: " + jsonResponse.optString("message", "Unknown error"));
                    }
                } else {
                    Log.e(TAG, "Payout Team Payouts API failed with response code: " + responseCode);
                }
            } catch (Exception e) {
                Log.e(TAG, "Error loading Payout Team payouts: " + e.getMessage());
                runOnUiThread(() -> {
                    Toast.makeText(PayoutTeamActivity.this, "Error loading payouts: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                });
            }
        });
    }
