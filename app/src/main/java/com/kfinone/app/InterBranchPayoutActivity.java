...
public class InterBranchPayoutActivity extends AppCompatActivity {
    private static final String TAG = "InterBranchPayoutActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_inter_branch_payout);
    ...
    private void initializeViews() {
        ...
        titleText.setText("Inter Branch Payout");
                        ...
                        runOnUiThread(() -> {
                            try {
                                ArrayAdapter<VendorBank> adapter = new ArrayAdapter<>(
                                    InterBranchPayoutActivity.this,
    ...
    private void loadPayoutList() {
        executor.execute(() -> {
            try {
                String urlString = BASE_URL + "get_inter_branch_payouts.php";
                URL url = new URL(urlString);
                HttpURLConnection connection = (HttpURLConnection) url.openConnection();
                connection.setRequestMethod("GET");
                connection.setConnectTimeout(10000);
                ...
                if (responseCode == HttpURLConnection.HTTP_OK) {
                    ...
                    Log.e(TAG, "Inter Branch Payouts API response: " + response.toString());
                    ...
                    if (jsonResponse.getBoolean("success")) {
                        ...
                        for (int i = 0; i < dataArray.length(); i++) {
                            ...
                            String payoutType = item.optString("payout_type_name", "Inter Branch Payout");
                        ...
                        Log.e(TAG, "Inter Branch Payouts API failed: " + jsonResponse.optString("message", "Unknown error"));
                    }
                } else {
                    Log.e(TAG, "Inter Branch Payouts API failed with response code: " + responseCode);
                }
            } catch (Exception e) {
                Log.e(TAG, "Error loading Inter Branch payouts: " + e.getMessage());
                runOnUiThread(() -> {
                    Toast.makeText(InterBranchPayoutActivity.this, "Error loading payouts: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                });
            }
        });
    }
