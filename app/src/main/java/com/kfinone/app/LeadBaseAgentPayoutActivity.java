...
public class LeadBaseAgentPayoutActivity extends AppCompatActivity {
    private static final String TAG = "LeadBaseAgentPayoutActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lead_base_agent_payout);
    ...
    private void initializeViews() {
        ...
        titleText.setText("Lead Base/Agent Payout");
                        ...
                        runOnUiThread(() -> {
                            try {
                                ArrayAdapter<VendorBank> adapter = new ArrayAdapter<>(
                                    LeadBaseAgentPayoutActivity.this,
    ...
    private void loadPayoutList() {
        executor.execute(() -> {
            try {
                String urlString = BASE_URL + "get_lead_base_agent_payouts.php";
                URL url = new URL(urlString);
                HttpURLConnection connection = (HttpURLConnection) url.openConnection();
                connection.setRequestMethod("GET");
                connection.setConnectTimeout(10000);
                ...
                if (responseCode == HttpURLConnection.HTTP_OK) {
                    ...
                    Log.e(TAG, "Lead Base/Agent Payouts API response: " + response.toString());
                    ...
                    if (jsonResponse.getBoolean("success")) {
                        ...
                        for (int i = 0; i < dataArray.length(); i++) {
                            ...
                            String payoutType = item.optString("payout_type_name", "Lead Base/Agent Payout");
                        ...
                        Log.e(TAG, "Lead Base/Agent Payouts API failed: " + jsonResponse.optString("message", "Unknown error"));
                    }
                } else {
                    Log.e(TAG, "Lead Base/Agent Payouts API failed with response code: " + responseCode);
                }
            } catch (Exception e) {
                Log.e(TAG, "Error loading Lead Base/Agent payouts: " + e.getMessage());
                runOnUiThread(() -> {
                    Toast.makeText(LeadBaseAgentPayoutActivity.this, "Error loading payouts: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                });
            }
        });
    }
