...
public class AddWorkIconsActivity extends AppCompatActivity {
    private static final String TAG = "AddWorkIconsActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_add_icons); // Reuse the same layout
        ...
        // Load existing work icons from database
        loadExistingWorkIcons();
    }
        ...
        executor.execute(() -> {
            try {
                ...
                Request request = new Request.Builder()
                    .url(BASE_URL + "add_work_icon.php")
                    .post(body)
                    .build();
                        ...
                        if (response.isSuccessful()) {
                            try {
                                ...
                                if (jsonResponse.getBoolean("success")) {
                                    Toast.makeText(AddWorkIconsActivity.this, 
                                        "Work icon added successfully!", Toast.LENGTH_SHORT).show();
                ...
                Log.e(TAG, "Error submitting work icon data", e);
                runOnUiThread(() -> {
                    submitButton.setEnabled(true);
                    submitButton.setText("Submit");
                    Toast.makeText(AddWorkIconsActivity.this, 
    ...
    private void loadExistingWorkIcons() {
        executor.execute(() -> {
            try {
                // Create request
                Request request = new Request.Builder()
                    .url(BASE_URL + "get_work_icons.php")
                    .get()
                    .build();
                ...
                try (Response response = client.newCall(request).execute()) {
                    String responseBody = response.body().string();
                    Log.d(TAG, "Work icons response: " + responseBody);
                            ...
                            try {
                                ...
                                if (jsonResponse.getString("status").equals("success")) {
                                    ...
                                    try {
                                        ...
                                        Log.d(TAG, "Loaded " + icons.size() + " work icons from database");
                                    } catch (Exception e) {
                                        Log.e(TAG, "Error parsing work icons data", e);
                                    }
                                } else {
                                    String error = jsonResponse.optString("message", "Unknown error occurred");
                                    Log.e(TAG, "Error loading work icons: " + error);
                                }
                            } catch (Exception e) {
                                Log.e(TAG, "Error parsing work icons response", e);
                            }
                        } else {
                            Log.e(TAG, "Server error loading work icons: " + response.code());
                        }
                    });
                }
            } catch (Exception e) {
                Log.e(TAG, "Error loading existing work icons", e);
            }
        });
    }
} 