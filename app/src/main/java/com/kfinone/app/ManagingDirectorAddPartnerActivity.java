    ...
    private void submitPartnerData() {
        ...
        executor.execute(() -> {
            try {
                ...
                jsonData.put("username", emailInput.getText().toString().trim());
                ...
                jsonData.put("Phone_number", phoneNoInput.getText().toString().trim());
                jsonData.put("email_id", emailInput.getText().toString().trim());
                jsonData.put("alternative_mobile_number", alternativePhoneInput.getText().toString().trim());
                
                // Company and address information
                jsonData.put("company_name", ""); // Will be set from form if available
                ...
                // Get selected spinner values (IDs)
                ...
                // Branch and bank information (IDs)
                jsonData.put("branch_state_name_id", selectedBranchState.getId());
                ...
                jsonData.put("reportingTo", "1"); // Default reporting to
                jsonData.put("employee_no", ""); // Will be generated or set
                jsonData.put("department", ""); // Will be set based on partner type
                jsonData.put("designation", ""); // Will be set based on partner type
                
                // Branch and bank names (text values for display)
                jsonData.put("branchstate", selectedBranchState.getName());
                jsonData.put("branchloaction", selectedBranchLocation.getName());
                jsonData.put("bank_name", selectedBank.getName());
                jsonData.put("account_type", selectedAccountType.getName());
                
                // Partner type
                jsonData.put("partner_type_id", selectedPartnerType.getId());
                
                // File uploads (for now, just placeholder strings)
                jsonData.put("pan_img", panCardUri != null ? panCardUri.toString() : "");
                jsonData.put("aadhaar_img", aadhaarCardUri != null ? aadhaarCardUri.toString() : "");
                jsonData.put("photo_img", photoUri != null ? photoUri.toString() : "");
                jsonData.put("bankproof_img", bankProofUri != null ? bankProofUri.toString() : "");
                
                // User and timestamp information
                jsonData.put("user_id", "1"); // Current user ID
                jsonData.put("created_at", new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new java.util.Date()));
