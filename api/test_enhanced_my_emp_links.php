...
try {
    ...
    // Check if this is K RAJESH KUMAR (User ID: 8)
    $isRajeshKumar = ($userId === '8');
    ...
    if ($manageTableCheck->num_rows > 0) {
        ...
        if ($manageStmt) {
            ...
            while ($row = $manageResult->fetch_assoc()) {
                if ($row['has_permission'] === 'Yes') {
                    $summary['manage_count']++;
                    $summary['total_count']++;
                }
                $manageIcons[] = [
                    'id' => $row['id'],
    ...
    $userInfo = [];
    $userQuery = "SELECT id, firstName, lastName, username, email_id, mobile, status 
