...
try {
    ...
    if (empty($selectColumns)) {
        throw new Exception('No valid columns found in tbl_data_icon');
    }

    $sql = "SELECT " . implode(', ', $selectColumns) . " FROM tbl_data_icon WHERE status = 1 OR status = 'Active' OR status IS NULL OR status = '' ORDER BY icon_name ASC";
