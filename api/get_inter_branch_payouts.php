...
try {
    ...
    // First, get the payout_type_id for "Inter Branch Payout"
    ...
    if (!$payoutTypeResult) {
        echo json_encode([
            'success' => false,
            'message' => 'Inter Branch Payout type not found',
            'data' => []
        ]);
        exit();
    }
    ...
    // Fetch Inter Branch Payout records with JOINs to get actual names
    $query = "
        SELECT 
            p.id,
            p.user_id,
    ...
    $stmt->bindParam(':payout_type_id', $interBranchPayoutTypeId, PDO::PARAM_INT);
    $stmt->execute();
    ...
    if ($payouts) {
        echo json_encode([
            'success' => true,
            'message' => 'Inter Branch Payout records fetched successfully',
            'data' => $payouts,
            'count' => count($payouts)
        ]);
    } else {
        echo json_encode([
            'success' => true,
            'message' => 'No Inter Branch Payout records found',
            'data' => [],
            'count' => 0
        ]);
    }
