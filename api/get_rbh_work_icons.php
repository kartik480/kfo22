<?php
// Regional Business Head Work Icons API
...
try {
    ...
    $table_check = $pdo->query("SHOW TABLES LIKE 'tbl_work_icon'");
    if ($table_check->rowCount() == 0) {
        throw new Exception("Table 'tbl_work_icon' does not exist");
    }
    
    // Fetch user data
    $userQuery = "SELECT work_icons FROM tbl_user WHERE id = ?";
    ...
    $workIcons = $userData['work_icons'];
    if (empty($workIcons)) {
        echo json_encode([
            'status' => 'success',
            'message' => 'No work icons assigned to this user',
            'data' => [],
            'debug_info' => [
                'user_id' => $userId,
                'work_icons' => $workIcons,
                'tables_exist' => true,
                'database_connected' => true,
                'input_received' => $input,
                'debug_input' => $debug_input
    ...
    // Parse work_icons - Handle both comma-separated and JSON array formats
    $iconIds = [];
    
    // Check if work_icons is a JSON array
    if (strpos($workIcons, '[') === 0 && strpos($workIcons, ']') === strlen($workIcons) - 1) {
        // Parse as JSON array
        $jsonArray = json_decode($workIcons, true);
        if (json_last_error() === JSON_ERROR_NONE && is_array($jsonArray)) {
            $iconIds = $jsonArray;
        } else {
            throw new Exception("Failed to parse JSON array from work_icons: " . json_last_error_msg());
        }
    } else {
        // Parse as comma-separated string
        $iconIds = explode(',', $workIcons);
    ...
    // Fetch icons from tbl_work_icon
