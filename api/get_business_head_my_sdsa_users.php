...
try {
    ...
    if (isset($loggedInUser['id'])) {
        $userIdQuery = "SELECT id FROM tbl_user WHERE id = ?";
        $userIdParams = [$loggedInUser['id']];
    } else {
        $userIdQuery = "SELECT id FROM tbl_user WHERE username = ?";
    ...
    // Now fetch all users from tbl_sdsa_users where reportingTo matches the logged-in user ID
    $query = "SELECT id, username, alias_name, first_name, last_name, password, Phone_number, 
                     ...
                     employee_no, department, designation, branchstate, branchloaction, bank_name, 
                     account_type, pan_img, aadhaar_img, photo_img, bankproof_img, user_id, 
    ...
    $totalSdsaUsersQuery = "SELECT COUNT(*) as total FROM tbl_sdsa_users";
    $totalSdsaUsersStmt = $conn->prepare($totalSdsaUsersQuery);
    $totalSdsaUsersStmt->execute();
    $totalSdsaUsers = $totalSdsaUsersStmt->fetch(PDO::FETCH_ASSOC)['total'];

    $reportingToCountQuery = "SELECT COUNT(*) as total FROM tbl_sdsa_users WHERE reportingTo = ?";
    $reportingToCountStmt = $conn->prepare($reportingToCountQuery);
    $reportingToCountStmt->execute([$loggedInUserId]);
    ...
    echo json_encode([
        ...
        'debug_info' => [
            ...
            'database_stats' => [
                'total_sdsa_users' => $totalSdsaUsers,
                'users_reporting_to_logged_in' => $reportingToCount,
                'sdsa_users_found' => count($users)
            ],
