...
try {
    ...
    // Get the payout_type_id for "Inter Branch Payout"
    $payoutTypeQuery = "SELECT id FROM tbl_payout_type WHERE payout_name = 'Inter Branch Payout' AND status = 1";
    $payoutTypeStmt = $pdo->prepare($payoutTypeQuery);
    $payoutTypeStmt->execute();
    $payoutTypeResult = $payoutTypeStmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$payoutTypeResult) {
        throw new Exception('Inter Branch Payout type not found in the system');
    }
    
    $interBranchPayoutTypeId = $payoutTypeResult['id'];
    ...
    $duplicateCheckStmt->execute([
        ':user_id' => $userId,
        ':payout_type_id' => $interBranchPayoutTypeId,
    ...
    if ($duplicateCheckStmt->rowCount() > 0) {
        throw new Exception('An Inter Branch Payout record already exists for this combination of user, loan type, vendor bank, and category');
    }
    
    // Insert the Inter Branch Payout
    $stmt = $pdo->prepare("
    ...
    echo json_encode([
        'status' => 'success',
        'message' => 'Inter Branch Payout added successfully',
        'data' => $insertedRecord
    ]);
