...
try {
    ...
    // TEMPORARY: Skip CBO verification for testing
    ...
    // TEMPORARY: Skip CBO verification - just check if user exists
    // $isCBO = ($loggedInUser['designation_name'] === 'Chief Business Officer');
    // if (!$isCBO) {
    //     throw new Exception('Access denied. Only Chief Business Officer users can access this feature.');
    // }
    
    // Query to fetch SDSA users reporting to this user
    $query = "SELECT 
                id,
                username,
                alias_name,
    ...
    echo json_encode([
        'success' => true,
        'message' => 'SDSA users fetched successfully (TEST MODE - CBO verification disabled)',
        'logged_in_user' => [
            'id' => $loggedInUser['id'],
