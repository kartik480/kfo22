...
try {
    ...
    // Build the main query to fetch portfolios created by CBO, RBH, and Director users
    $sql = "SELECT 
                p.id,
                p.customer_name,
                p.company_name,
    ...
    // Get unique creators by designation
    ...
    // Group creators by designation
    $creatorsByDesignation = [];
    foreach ($creators as $creator) {
        $designation = $creator['designation_name'];
        if (!isset($creatorsByDesignation[$designation])) {
            $creatorsByDesignation[$designation] = [];
        }
        $creatorsByDesignation[$designation][] = $creator;
    }
    
    // Format the response
    $response = [
        'success' => true,
        'message' => 'Portfolios created by CBO, RBH, and Director users fetched successfully',
        'data' => [
            'designation_info' => $designations,
            'portfolios' => $portfolios,
            'creators' => $creators,
            'creators_by_designation' => $creatorsByDesignation,
            'statistics' => [
                'total_portfolios' => $totalCount,
                'status_counts' => $statusCounts,
                'total_creators' => count($creators),
                'designation_counts' => array_map('count', $creatorsByDesignation)
            ]
        ],
        'count' => $totalCount
    ];
