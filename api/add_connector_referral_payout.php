...
try {
    ...
    // Get the payout_type_id for "Connector/Referral Payout"
    $payoutTypeQuery = "SELECT id FROM tbl_payout_type WHERE payout_name = 'Connector/Referral Payout' AND status = 1";
    $payoutTypeStmt = $pdo->prepare($payoutTypeQuery);
    $payoutTypeStmt->execute();
    $payoutTypeResult = $payoutTypeStmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$payoutTypeResult) {
        throw new Exception('Connector/Referral Payout type not found in the system');
    }
    
    $connectorReferralPayoutTypeId = $payoutTypeResult['id'];
    ...
    $duplicateCheckStmt->execute([
        ':user_id' => $userId,
        ':payout_type_id' => $connectorReferralPayoutTypeId,
    ...
    if ($duplicateCheckStmt->rowCount() > 0) {
        throw new Exception('A Connector/Referral Payout record already exists for this combination of user, loan type, vendor bank, and category');
    }
    
    // Insert the Connector/Referral Payout
    $stmt = $pdo->prepare("
    ...
    echo json_encode([
        'status' => 'success',
        'message' => 'Connector/Referral Payout added successfully',
        'data' => $insertedRecord
    ]);
