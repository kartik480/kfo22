...
try {
    ...
    // First, get the payout_type_id for "Branch/Full Payout"
    $payoutTypeQuery = "SELECT id FROM tbl_payout_type WHERE payout_name = 'Branch/Full Payout' AND status = 1";
    ...
    if (!$payoutTypeResult) {
        echo json_encode([
            'success' => false,
            'message' => 'Branch/Full Payout type not found',
            'data' => []
        ]);
        exit();
    }
    
    $branchFullPayoutTypeId = $payoutTypeResult['id'];
    
    // Fetch Branch/Full Payout records with JOINs to get actual names
    $query = "
        SELECT 
            p.id,
            p.user_id,
    ...
    $stmt->bindParam(':payout_type_id', $branchFullPayoutTypeId, PDO::PARAM_INT);
    $stmt->execute();
    ...
    if ($payouts) {
        echo json_encode([
            'success' => true,
            'message' => 'Branch/Full Payout records fetched successfully',
            'data' => $payouts,
            'count' => count($payouts)
        ]);
    } else {
        echo json_encode([
            'success' => true,
            'message' => 'No Branch/Full Payout records found',
            'data' => [],
            'count' => 0
        ]);
    }
