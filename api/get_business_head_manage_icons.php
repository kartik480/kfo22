...
try {
    ...
    if (isset($loggedInUser['id'])) {
        $query = "SELECT manage_icons FROM tbl_user WHERE id = ?";
        $params = [$loggedInUser['id']];
    } else {
        $query = "SELECT manage_icons FROM tbl_user WHERE username = ?";
    ...
    $manageIcons = $userResult['manage_icons'];
    
    if (empty($manageIcons)) {
        echo json_encode([
            'status' => 'success',
            'message' => 'No manage icons found for this user',
            'data' => [],
            'count' => 0,
            'debug_info' => [
    ...
    // Parse manage_icons - check if it's JSON or comma-separated
    $iconIds = [];
    
    // Try to parse as JSON first
    $jsonIcons = json_decode($manageIcons, true);
    if (json_last_error() === JSON_ERROR_NONE && is_array($jsonIcons)) {
        $iconIds = $jsonIcons;
    } else {
        // Parse as comma-separated string
        $iconIds = array_filter(array_map('trim', explode(',', $manageIcons)));
    }
    
    if (empty($iconIds)) {
        throw new Exception("No valid icon IDs found in manage_icons");
    }
    ...
    $totalManageIconsQuery = "SELECT COUNT(*) as total FROM tbl_manage_icon";
    $totalManageIconsStmt = $conn->prepare($totalManageIconsQuery);
    $totalManageIconsStmt->execute();
    $totalManageIcons = $totalManageIconsStmt->fetch(PDO::FETCH_ASSOC)['total'];
    ...
    echo json_encode([
        ...
        'debug_info' => [
            ...
            'database_stats' => [
                ...
                'total_manage_icons' => $totalManageIcons,
    ...
    echo json_encode([
        'status' => 'error',
        'message' => 'Failed to fetch manage icons: ' . $e->getMessage(),
        'debug_info' => [
