...
try {
    ...
    // Get all partner users from tbl_partner_users who are reporting to ID 11
    $sql = "SELECT 
    ...
    echo json_encode(array(
        'status' => 'success',
        'data' => $partnerUsers,
        'message' => 'Partner users reporting to ID 11 fetched successfully',
        'count' => count($partnerUsers),
        'debug' => array(
            'sql_used' => $sql,
            'reporting_to_id' => '11',
            'note' => 'Partner users from tbl_partner_users reporting to ID 11'
        )
    ));
    
} catch (Exception $e) {
    error_log("Error in get_director_partner_users_reporting_to_11.php: " . $e->getMessage());
