...
try {
    ...
    if (empty($fields)) {
        throw new Exception('No valid columns found in tbl_data_icon table');
    }

    $sql = "INSERT INTO tbl_data_icon (" . implode(', ', $fields) . ") VALUES (" . implode(', ', $placeholders) . ")";
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Data icon added successfully',
            'icon_id' => $icon_id,
            'data' => [
                'icon_name' => $icon_name,
                'icon_url' => $icon_url,
        ...
        throw new Exception('Failed to insert data icon: ' . $stmt->error);
    }
