...
try {
    ...
    // TEMPORARY: Skip BH verification for testing
    ...
    // TEMPORARY: Skip BH verification - just check if user exists
    // $isBH = ($loggedInUser['designation_name'] === 'Business Head');
    // if (!$isBH) {
    //     throw new Exception('Access denied. Only Business Head users can access this feature.');
    // }
    
    // Query to fetch SDSA users reporting to this BH user
    $query = "SELECT 
                id,
                username,
                alias_name,
    ...
    echo json_encode([
        'success' => true,
        'message' => 'SDSA users fetched successfully (TEST MODE - BH verification disabled)',
        'logged_in_user' => [
            'id' => $loggedInUser['id'],
