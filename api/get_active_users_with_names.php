...
try {
    ...
    // Build the query with LEFT JOINs if tables exist
    $sql = "SELECT 
                u.id,
                u.firstName,
                u.lastName,
    ...
    // Filter for active users (status = 1 or other active values)
    $sql .= " WHERE u.status = 1 OR u.status = 'Active' OR u.status IS NULL OR u.status = ''";
    ...
    echo json_encode([
        'status' => 'success',
        'message' => 'All users fetched successfully with designation and department names',
        'data' => $users,
        'summary' => [
