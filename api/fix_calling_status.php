...
try {
    ...
    // Now fetch all calling statuses
    $stmt = $conn->prepare("SELECT id, calling_status FROM tbl_partner_calling_status ORDER BY calling_status ASC");
    $stmt->execute();
    $callingStatuses = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Return the data in the format expected by Android app
    ...
    error_log("Fix calling status error: " . $e->getMessage());
