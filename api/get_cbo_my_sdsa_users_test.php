...
try {
    ...
    // For testing, we'll skip the CBO verification and just check if the user exists
    $userVerificationSql = "
    ...
    // Query to fetch SDSA users reporting to this user (for testing, we'll allow any user)
    $query = "SELECT 
                id,
                username,
                alias_name,
    ...
    echo json_encode([
        'success' => true,
        'message' => 'SDSA users fetched successfully (TEST MODE - No CBO verification)',
        'logged_in_user' => [
            'id' => $loggedInUser['id'],
