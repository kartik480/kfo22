...
try {
    ...
    // Build the query to get work_icons from tbl_user
    $userQuery = "SELECT work_icons FROM tbl_user WHERE ";
    $userParams = [];
    ...
    $work_icons = $userResult['work_icons'];
    
    // If work_icons is empty or null, return empty array
    if (empty($work_icons)) {
        echo json_encode([
            'status' => 'success',
            'data' => [],
    ...
    // Parse the work_icons (assuming it's comma-separated IDs)
    $icon_ids = explode(',', $work_icons);
    ...
    // Build the query to get icon details from tbl_work_icon
    ...
    echo json_encode([
        'status' => 'success',
        'data' => $formattedData,
        'message' => 'Work icons retrieved successfully',
